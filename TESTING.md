...
src/
  app/
    theme.test.tsx                   # Camada 2 — ThemeProvider aplica html.dark
  ...
  pages/
    auth/
      LoginPage.test.tsx             # Camada 2 — contrato dark das telas auth
    admin/
  ...
  shared/
    ui/
      ...
      Header.test.tsx                # Camada 2
      Modal.test.tsx                 # Camada 2
      PageMeta.test.tsx              # Camada 2 — title/meta/canonical por rota pública
...
O workflow `.github/workflows/test.yml` roda apenas em PRs direcionados à `main` e quando chamado pelo deploy via `workflow_call`. Push em branch comum não dispara testes, para economizar minutos do GitHub Actions.

| Trigger | Job `unit` | Job `integration` |
|---|---|---|
| Push em branch comum | — | — |
| Pull request para `main` | ✓ | ✓ |
| Push em `main` (via `deploy.yml`) | ✓ | ✓ |

### Job `unit`
Roda `npm test` (Vitest, Camadas 1–3) e `npm run coverage`.

### Job `integration`
Roda apenas em PR e antes do deploy:
1. Instala o Supabase CLI via `supabase/setup-cli`.
2. `supabase start` + `supabase db reset` (aplica todas as migrations e grants).
...
### Deploy
`deploy.yml` chama `test.yml` via `workflow_call` com `secrets: inherit`. O job `deploy` declara `needs: test`, portanto o deploy só acontece se ambos os jobs passarem.
...
|---|---|---|---|---|---|---|
| `shared/ui` (Button, Card, Modal, Field, Header, PageMeta, Skeleton, StateMessage) | — | 🟢 | — | — | — | T-04 + F6-01/F6-02: Header mobile, altura tocável de Button e scroll responsivo do Modal; F6-04: PageMeta title/meta/canonical; F6-05: skip link, menu `aria-controls`, foco/trap/restauração do Modal e descrição de Field |
...
| `features/auth` — AdminGuard/telas auth/tema | — | 🟢 | — | — | — | T-04 + F6-01: redirecionamentos, loading e erro; F6-03: ThemeProvider aplica `html.dark` e LoginPage cobre contrato dark; F6-05: LoginPage cobre labels acessíveis |
...
F6-02 adicionou cobertura de componente para contratos responsivos compartilhados (`Header`, `Button`, `Modal`) e uma verificação Playwright local de overflow horizontal nas rotas `/`, `/adocao`, `/historias`, `/eventos`, `/admin/login`, `/admin/enroll`, `/admin/verify` e `/admin/definir-senha`, em 375px e 1280px. Resultado esperado: `overflow=0` em todas.

### Verificação de tema

F6-03 adicionou `src/app/theme.test.tsx` para garantir que o `ThemeProvider` alterna `html.dark` e persiste `localStorage.theme`, além de `src/pages/auth/LoginPage.test.tsx` para travar o contrato dark das superfícies/campos/botão usados pelas telas de autenticação. Também foi feita uma checagem Playwright headless local nas rotas `/`, `/adocao`, `/historias`, `/eventos`, `/admin/login`, `/admin/enroll`, `/admin/verify` e `/admin/definir-senha`, confirmando `html.dark=true`, `body` escuro e texto claro.

### Verificação de SEO

F6-04 adicionou `src/shared/ui/PageMeta.test.tsx`, cobrindo atualização de `document.title`, `meta[name="description"]`, Open Graph e canonical. O build também confirma que `public/favicon.svg`, `site.webmanifest`, `robots.txt` e `sitemap.xml` são copiados para `dist/`.

### Verificação de acessibilidade

F6-05 ampliou testes de componente para teclado e semântica: `Header.test.tsx` cobre skip link e `aria-controls` do menu mobile; `Modal.test.tsx` cobre foco inicial, restauração de foco e ciclo de Tab dentro do dialog; `Field.test.tsx` cobre `aria-describedby`/`aria-invalid`; `LoginPage.test.tsx` cobre labels acessíveis nos campos de login.
